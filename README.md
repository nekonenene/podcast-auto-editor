...
4. 日本語をローカルで文字起こしして（誰が喋ったかのラベルも付けられます）
...
--diarize           文字起こしへ話者ラベルを付ける
--speakers 2        収録に参加している人数
```

### 話者ラベル（話者分離）

`--diarize` を付けると、文字起こしの各発言へ「話者1」「話者2」のラベルが付きます。  
ブログ記事などを起こすときに、誰の発言かがわかると読み違えが減ります。

参加人数は `--speakers` で指定してください。自動では推定しません。  
人数を取り違えると同じ人が2人に割れてしまい、いちばん困る形で崩れるためです。

```bash
cargo run -p pae-cli -- run input.mp4 -o output --diarize --speakers 2
```

番号は最初に喋った人が「話者1」になります。  
声が被っている箇所や短い相槌など、決めきれなかった発言には「話者不明」が付きます。

1トラックへミックス済みの録音では、被って話した部分を分けきれません。  
そのままの完成品ではなく、人が直す前提の下書きとして使ってください。
...
話者分離を使う場合は、話者の声質を表すモデル (約26MB) も初回に自動でダウンロードされます。
