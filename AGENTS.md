...
コメントやコミットメッセージは、ジュニアエンジニアでもわかりやすい言葉を選び、以下の手法で読みやすさを追求する。

- 改行は基本的に句点や読点の後におこなう。多少１行が長くなっても、文章の意味が途切れないことを優先する
- カッコを過度に使わない。文章のリズムが悪くなり読みにくくなるため（改善例：「教師（教育のプロ）が参加する」→ 「教育のプロである教師が参加する」）
- 英語直訳による馴染みの薄い単語は避け、わかりやすく言い換える（改善例：「最外トランザクション」→ 「もっとも外側のトランザクション」）
- 「ユーザ」でなく「ユーザー」などの表記をおこなう。一般的な表記を優先する
