...
pub fn print_report(report: &JobReport) {
    ...
    println!(
        "入力: {} / 出力: {} (短縮率 {:.1}%)",
        format_duration_ms(report.source_duration_ms),
        format_duration_ms(report.output_duration_ms),
        100.0 * (1.0 - report.output_duration_ms as f64 / report.source_duration_ms as f64)
    );
    println!();
    println!("処理時間:");
